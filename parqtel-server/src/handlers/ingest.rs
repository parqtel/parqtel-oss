...
pub async fn ingest_proto(State(state): State<AppState>, body: Bytes) -> Response {
    ...
    match state.inner.ingestion_service.ingest_proto(body).await {
        ...
        Err(e) => { state.inner.metrics.ingest_errors.fetch_add(1, std::sync::atomic::Ordering::Relaxed); map_error(e) }
    }
}
...
fn map_error(e: parqtel_core::Error) -> Response {
    let status = match &e {
        parqtel_core::Error::Validation(_) | parqtel_core::Error::InvalidOtlp(_) => StatusCode::BAD_REQUEST,
        _ => StatusCode::INTERNAL_SERVER_ERROR,
    };
    (status, Json(json!({"status": "error", "error": e.to_string()}))).into_response()
}
