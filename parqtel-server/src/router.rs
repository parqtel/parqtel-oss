...
pub fn build_router(state: AppState) -> Router {
    ...
    let routes = Router::new()
        ...
        .route("/api/v1/query_range", get(handlers::prometheus::query_range))
        .route("/api/v1/logs", get(handlers::prometheus::query_logs))
        .route("/api/v1/label/__name__/values", get(handlers::prometheus::list_metric_names))
        .route("/api/v1/label/:name/values", get(handlers::prometheus::list_label_values))
        .route("/api/v1/labels", get(handlers::prometheus::list_label_names))

        // Log API
        .route("/v1/logs/count", get(handlers::prometheus::query_logs_count))
        .route("/v1/logs/fields", get(handlers::prometheus::list_log_fields))
        .route("/v1/logs/field_values", get(handlers::prometheus::list_log_field_values))

        // Correlation & Traces
        .route("/v1/correlate", get(handlers::prometheus::correlate))
        .route("/v1/traces/search", get(handlers::prometheus::search_traces))
        ...
        .route("/api/v1/alerts/:id/acknowledge", post(handlers::alerts::acknowledge_alert))
        .route("/api/v1/alerts/:id/resolve", post(handlers::alerts::resolve_alert))
    ...
    routes
        ...
        .layer(TimeoutLayer::new(Duration::from_secs(query_config.timeout_secs)))
