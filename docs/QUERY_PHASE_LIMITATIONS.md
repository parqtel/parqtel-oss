...
4. **Instant lookback is now configurable** — `query.lookback_delta_ns` (default 300000000000 = 5 minutes, the Prometheus default) drives the instant-selector window via `with_query_lookback`; benchmark datasets older than the window still return empty on instant queries by design (`topk`, `instant_selector` show 0 rows in the baseline for this reason — data is ~35–60 min old). The earlier fixed 1-minute window was superseded when the lookback became config-driven.
