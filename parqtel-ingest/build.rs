...
fn main() -> Result<()> {
    ...
    let resource_proto = Path::new(proto_dir).join("opentelemetry/proto/resource/v1/resource.proto");
    let collector_metrics_proto = Path::new(proto_dir).join("opentelemetry/proto/collector/metrics/v1/metrics_service.proto");
    
    // New log protos
    let logs_proto = Path::new(proto_dir).join("opentelemetry/proto/logs/v1/logs.proto");
    let collector_logs_proto = Path::new(proto_dir).join("opentelemetry/proto/collector/logs/v1/logs_service.proto");
    
    // Trace protos
    let traces_proto = Path::new(proto_dir).join("opentelemetry/proto/trace/v1/trace.proto");
    let collector_traces_proto = Path::new(proto_dir).join("opentelemetry/proto/collector/trace/v1/trace_service.proto");

    if metrics_proto.exists() && common_proto.exists() && resource_proto.exists() && 
