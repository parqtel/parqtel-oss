...
fn convert_span(proto: ProtoSpan, _resource_attributes: LabelSet) -> Result<Span> {
    ...
    if proto.parent_span_id.len() == 8 {
        parent_span_id.copy_from_slice(&proto.parent_span_id);
    }
    ...
    let events = proto
        ...
        .map(|e| {
            ...
            .unwrap_or_default();
            SpanEvent {
                time_ns: e.time_unix_nano as i64,
                name: e.name,
                attributes: attrs,
            }
        })
    ...
    let links = proto
        ...
        .map(|l| {
            let mut tid = [0u8; 16];
            if l.trace_id.len() == 16 {
                tid.copy_from_slice(&l.trace_id);
            }
            let mut sid = [0u8; 8];
            if l.span_id.len() == 8 {
                sid.copy_from_slice(&l.span_id);
            }
    ...
    Ok(Span::new(
        ...
        proto.name,
        proto.kind,
        proto.start_time_unix_nano as i64,
        proto.end_time_unix_nano as i64,
        attributes,
        events,
        links,
        status,
...
fn json_to_span(sp: &serde_json::Value) -> Result<Span> {
        ...
        evts.iter()
            .map(|e| {
                let attrs = e
                    .get("attributes")
                    .and_then(|a| a.as_array())
                    .map(|a| json_attrs_to_labels(a).unwrap_or_default())
                    .unwrap_or_default();
                SpanEvent {
                    time_ns: parse_json_timestamp(
                        e.get("time_unix_nano").or(e.get("timeUnixNano")),
                    )
                    .unwrap_or(0),
                    name: e
                        .get("name")
    ...
    let start_ns = parse_json_timestamp(
        sp.get("start_time_unix_nano")
            .or(sp.get("startTimeUnixNano")),
    )?;
    let end_ns = parse_json_timestamp(sp.get("end_time_unix_nano").or(sp.get("endTimeUnixNano")))?;
    let trace_state = sp
        .get("trace_state")
        .or(sp.get("traceState"))
        .and_then(|v| v.as_str())
        .unwrap_or_default()
        .to_string();
